...
export class LeaderboardComponent {
    ...
    readonly leaderboard = computed(() => {
        ...
        const leaderboard = players.map((player) => {
            const objectNodes = nodes.filter((node): node is { id: string, value: string } => Boolean(node && typeof node === 'object' && 'id' in node && 'value' in node));
            const node = objectNodes.find((node) => node.id === locations.find((location) => location.players.includes(player))?.id);
